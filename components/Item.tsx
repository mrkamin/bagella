        ...
        <div className="absolute w-full bottom-0  duration-500 ease-in-out flex justify-between p-2 bg-white/80 z-20">
          <button onClick={handleAddToCart} aria-label="Add to cart" className="relative cursor-pointer group">
            ...
            {totalItems > 0 && (
              ...
              <div className="absolute -top-8 left-[3%] opacity-0 group-hover:opacity-100 transition-opacity duration-500 text-white bg-black/80 text-xs px-2 py-1 rounded z-30 whitespace-nowrap">Add To Cart</div>
          </button>
          <button aria-label="Add to wishlist" className="cursor-pointer group">
            <Heart className='text-primary hover:text-input' />
            <div className="absolute -top-8 left-[25%] opacity-0 group-hover:opacity-100 transition-opacity duration-500 text-white bg-black/80 text-xs px-2 py-1 rounded z-30 whitespace-nowrap">Add To Favorait</div>
          </button>
          <button aria-label="Contact on WhatsApp" className="cursor-pointer group" >
            ...
            <div className="absolute -top-8 right-[20%] opacity-0 group-hover:opacity-100 transition-opacity duration-500 text-white bg-black/80 text-xs px-2 py-1 rounded z-30 whitespace-nowrap">Order On Whatsapp</div>
          </button>
          <BuyDialog id={product._id.toString()}>
            <button className="cursor-pointer group">
              <ShoppingBag className='text-primary hover:text-input' />
              <div className="absolute -top-8 right-[3%] opacity-0 group-hover:opacity-100 transition-opacity duration-500 text-white bg-black/80 text-xs px-2 py-1 rounded z-30 whitespace-nowrap">Buy Now</div>
            </button>
          </BuyDialog>
        </div>
      </div>
